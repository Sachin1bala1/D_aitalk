...
mod tests {
    ...
    fn test_csv_load_and_query() {
        let engine = DuckDbEngine::new().expect("DuckDB init");
        let csv = "C:/Users/sachi/Documents/manufacturing_agent/daitalk-v2/historian_export.csv";

        // Load CSV
        engine.load_csv(csv, "historian").expect("load_csv failed");

        // Verify view exists
        let views = engine.list_views().expect("list_views failed");
        assert!(views.contains(&"historian".to_string()), "view not found: {views:?}");

        // Query row count
        let conn = engine.conn.lock().unwrap();
        let count: i64 = conn
            .query_row("SELECT COUNT(*) FROM historian", duckdb::params![], |r| r.get(0))
            .expect("count query failed");
        assert_eq!(count, 2000, "expected 2000 rows, got {count}");

        // Query distinct tags
        let mut stmt = conn.prepare("SELECT COUNT(DISTINCT Tag) FROM historian").unwrap();
        let tag_count: i64 = stmt.query_row(duckdb::params![], |r| r.get(0)).unwrap();
        assert_eq!(tag_count, 4, "expected 4 tags");

        // Time-series query on REACTOR.TEMP
        let mut stmt = conn
            .prepare("SELECT AVG(Value) FROM historian WHERE Tag = 'REACTOR.TEMP'")
            .unwrap();
        let avg: f64 = stmt.query_row(duckdb::params![], |r| r.get(0)).unwrap();
        assert!((90.0..110.0).contains(&avg), "avg {avg} out of expected range");

        println!("PASS: {count} rows, {tag_count} tags, REACTOR.TEMP avg={avg:.2}");
    }
}
